...
@section('content')
	<div class="wrap">
		...
		<div class="container inner-wrap">
			...
			{{ method_field('PUT') }}
			<div class="title">
				<input type="text" placeholder="标题" class="form-control title-input" value="{{ $article->title }}">
			</div>
			<div class="content">
...
<script>
	console.log(markdown.toHTML('**dddd**'));
	$(function () {
		$_token = $('input[name="_token"]').val();
		$_method = $('input[name="_method"]').val();
		
		$parsedStr = parse('{{ $article->content }}');
		$dom = $.parseHTML($parsedStr);
